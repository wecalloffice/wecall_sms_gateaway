...
import { Users, UserPlus, Activity, TrendingUp, Search, Edit, Trash2 } from 'lucide-react';
...
export default function ResellerClientsPage() {
  ...
  const fetchClients = async () => {
    try {
      ...
      setClients(reseller.clients || []);
      setFilteredClients(reseller.clients || []);
      setLoading(false);
    } catch (error) {
  ...
  const handleAddClient = async (data: any) => {
    try {
      if (!currentReseller) throw new Error('No reseller selected');
      await mockAccounts.createClient(data, currentReseller.account_sid);
  ...
  const handleEditClient = async (data: any) => {
    try {
      await mockAccounts.updateClient(selectedClient.account_sid, data);
  ...
  const handleDeleteClient = async (accountSid: string, name: string) => {
    if (!confirm(`Are you sure you want to delete client "${name}"?`)) return;

    try {
      await mockAccounts.deleteClient(accountSid);
  ...
  return (
    <MainLayout role="RESELLER_ADMIN" businessName="KCB Bank" userName="KCB Admin">
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600">Credit Limit Total</p>
                <p className="text-2xl font-bold text-gray-900">
                  ${clients.reduce((sum, c) => sum + (c.credit_limit || 0), 0).toFixed(2)}
                </p>
              </div>
        ...
        <div className="bg-white rounded-lg shadow border border-gray-200">
          <div className="p-6 border-b border-gray-200">
            <h2 className="text-lg font-semibold text-gray-900">Client List ({filteredClients.length})</h2>
          </div>
                ...
                {loading ? (
                  ...
                  filteredClients.map((client) => (
                    <tr key={client.account_sid} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-600">{client.account_sid}</td>
                      ...
                      <td className="px-6 py-4 text-sm space-x-2">
                        ...
                        <button
                          onClick={() => handleDeleteClient(client.account_sid, client.name)}
      ...
      <AddClientModal
        ...
        resellerName={currentReseller?.name || 'Your Reseller'}
      />
