...
export default function ResellerSmsPage() {
  ...
  // Apply filters
  useEffect(() => {
    let filtered = messages.filter(m => m.reseller_sid === 'AC_RESELLER_1001');
    if (filterStatus !== 'all') {
  ...
  const fetchMessages = async () => {
    try {
      const data = await mockSms.list({ reseller_sid: 'AC_RESELLER_1001' });
      setMessages(data);
      const statsData = await mockSms.getStats('AC_RESELLER_1001');
      setStats(statsData);
    } catch (err) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setFormData({ to: '', from: 'RESELLER_SMS', message: '' });
      await fetchMessages();
    } catch (err) {
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600 mt-1">Send individual or bulk SMS messages to your customers</p>
          </div>
        </div>
