...
export default function ResellerProfilePage() {
  ...
  const loadProfile = async () => {
    try {
      setLoading(true);
      const profileData = await mockProfile.getProfile('RESELLER_ADMIN');
      const statsData = await mockProfile.getAccountStats('AC_RESELLER_1001');
  ...
  const handleSaveProfile = async () => {
    try {
      const updated = await mockProfile.updateProfile('AC_RESELLER_1001', formData);
      setProfile(updated);
      setEditMode(false);
  ...
  if (loading) {
    return (
      <MainLayout role="RESELLER_ADMIN" businessName="Acme" userName="Reseller Admin">
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600 mt-1">Manage your reseller account and business settings</p>
          </div>
          {!editMode && (
            <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="bg-white rounded-lg shadow border border-gray-200 p-6">
            ...
            <p className="text-sm text-gray-600 mt-2">Direct clients</p>
          </div>

          {/* Active Sub-Resellers */}
          <div className="bg-white rounded-lg shadow border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-gray-900">Sub-Resellers</h3>
              <Zap size={24} className="text-green-600" />
            </div>
            <p className="text-2xl font-bold text-green-600">{stats?.active_sub_resellers}</p>
            <p className="text-sm text-gray-600 mt-2">Active resellers</p>
          </div>

          {/* Total SMS Sent */}
          <div className="bg-white rounded-lg shadow border border-gray-200 p-6">
            <h3 className="font-semibold text-gray-900 mb-4">Total SMS Sent</h3>
            <p className="text-2xl font-bold text-purple-600">{(stats?.total_sms_sent / 1000).toFixed(1)}K</p>
            <p className="text-sm text-gray-600 mt-2">Since inception</p>
          </div>

          {/* Account Balance */}
          <div className="bg-white rounded-lg shadow border border-gray-200 p-6">
            <h3 className="font-semibold text-gray-900 mb-4">Balance</h3>
            <p className="text-2xl font-bold text-green-600">${stats?.account_balance.toFixed(2)}</p>
            <button className="btn-primary w-full mt-2 py-1 text-sm">Top Up</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="flex items-center justify-between py-3 hover:bg-gray-50 px-2 rounded">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-gray-900">{profile.company_size}</p>
                      </div>
                    </div>
                  </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-lg shadow border border-gray-200 p-6">
              ...
              <button className="w-full btn-secondary mb-2 text-sm py-2">Change Password</button>
              <button className="w-full btn-secondary text-sm py-2">Enable 2FA</button>
            </div>
            ...
            <div className="bg-white rounded-lg shadow border border-gray-200 p-6">
              ...
              <div className="space-y-3">
                <div>
                  <p className="text-xs text-gray-600 uppercase font-semibold">Tax ID</p>
                  <p className="font-mono text-sm text-gray-900">{profile.tax_id}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-600 uppercase font-semibold">Registration #</p>
                  <p className="font-mono text-sm text-gray-900">{profile.registration_number}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow border border-gray-200 p-6">
              ...
              <p className="text-xs text-gray-500 mt-2">For API integration</p>
            </div>

            {/* Status Badge */}
            <div className="bg-green-50 border border-green-200 rounded-lg p-6">
              <p className="text-sm text-green-700 font-medium">✓ Account is active</p>
              <p className="text-xs text-green-600 mt-1">All features available</p>
            </div>
          </div>
        </div>
      </div>
