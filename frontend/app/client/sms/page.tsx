...
import { Send, MessageSquare, Loader, CheckCircle, AlertCircle, Clock } from 'lucide-react';
...
export default function ClientSmsPage() {
  ...
  const fetchMessages = async () => {
    try {
      ...
      setMessages(data);
      const statsData = await mockSms.getStats('AC_CLIENT_001');
      setStats(statsData);
    } catch (err) {
  ...
  const handleSend = async () => {
    ...
    try {
      // Check if it's bulk (contains commas)
      ...
      setFormData({ to: '', from: 'SMS_SENDER', message: '' });
      await fetchMessages();
    } catch (err) {
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600 mt-1">Send individual or bulk SMS messages</p>
          </div>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                disabled={loading}
              >
                <option value="SMS_SENDER">SMS_SENDER</option>
